...
export const setSetting = async (info) => {
  const _data = { ...info };
  await localStore.store_data("setting", _data);
  return true;
};
export const getUserInfo = () => localStore.get_data("userinfo");

export const get_Setting = () => localStore.get_data("setting");
...
export const getCategories = async () => {
  ...
  return filter(_userInfo.categories, (item) => item._id);
};
